...
Mozyfin cung cấp **hai chat workflows chính** và chế độ **auto routing**:
...
<Tip>
    **Mode `auto`**: Hệ thống tự động phân tích độ phức tạp của câu hỏi và routing đến Flash Chat hoặc Simple Chat phù hợp. Đây là mode mặc định, được khuyến nghị cho hầu hết các trường hợp sử dụng.
</Tip>
...
Sử dụng mode **`auto`** (mặc định) để hệ thống tự động routing dựa trên độ phức tạp của câu hỏi. Bảng dưới đây minh họa cách auto routing hoạt động:

| Tình huống                                                   | Auto routing đến    |
| ---------------------------------------------------------- | -------------------- |
